  ...
  fetch(url)
    ...
    .then((json) => {
      ...
      for (let j = 0; j < bookingIds.length; j++) {
        const el = document.getElementById(bookingIds[j]);
        el.addEventListener('click', function () {
          const theId = el.id.replace('delete', '');
          deleteBooking(theId);
        });
      }
    });
...
function deleteBooking(id) {
  let url =
    'https://api.sheety.co/0147939e4fd694bb5d591f2eddf97947/bookingApp/bookings/' + id;
  fetch(url, {
    method: 'DELETE',
  })
    .then(() => {
